...
[p,q] = size(no_delay);
[u,i] = size(delay);
tr_req = (p/3)*2;tr_req1 = (u/3)*2;tr_req = fix(tr_req);tr_req1 = fix(tr_req1);
g_delay = ones*ones(u,1);
g_no_delay = zeros(p,1);
flight_no_training = [no_delay(1:tr_req,9);delay(1:tr_req1,9)];
flight_no_testing = [no_delay(tr_req+1:end,9);delay(tr_req1+1:end,9)];
training = [no_delay(1:tr_req,1:8);delay(1:tr_req1,1:8)];
testing = [no_delay(tr_req+1:end,1:8);delay(tr_req1+1:end,1:8)];
...
for i = 1:lo
 ...
 pr1 = con(1,1)+con(2,2);
 arrival_delay_for_flights = [flight_no_testing classification];
 ...
 disp('The arrival delay classification corresponding to each of flight number can be found in the variable " arrival_delay_for_flights".');
 disp('First column indicates the flight number and the second column indiactes the delay classification');
 fprintf('\n');
...
trai_dat= fix(trai_dat);
%getting the training data
training_reg = A(1:trai_dat,2:3);
...
testing_reg = A(trai_dat+1:end,2:3);
...
% The variable prediction contains the flight arrival delay prediction for all the test data
   prediction = round(prediction,0);
    ...
    flight_number_train = A(1:trai_dat,4);flight_number_test = A(trai_dat+1:end,4);
    extent_of_delay_for_flight = [flight_number_test prediction];
    ...
    disp('The original correct flight delay values for the test data can be found in the variable named "y1"');
     disp('The extent of delay corresponding to each of the flight number is found in the variable "extent_of_delay_for_flight"');
     disp('First column indicates the flight number and second column indicates the extent of delay');
    time_taken = toc;